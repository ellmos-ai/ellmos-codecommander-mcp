...
export const de: Translations = {
  ...
  cc_analyze_methods: {
    ...
    guardrailsHeader: '**BACH-Prüfungen:**',
    missingSignalCallbacksHeader: 'Fehlende Signal-Callbacks:',
    missingSignalCallback: (line, method) => `  Z.${line}: self.${method}-Callback nicht gefunden`,
    attributeIssuesHeader: 'Attribut-Reihenfolge:',
    attributeNeverDefined: (line, attr) => `  Z.${line}: self.${attr} verwendet, aber nie definiert`,
    attributeBeforeDefinition: (useLine, attr, defLine) =>
      `  Z.${useLine}: self.${attr} vor Definition in Z.${defLine} verwendet`,
    underscoreMismatchesHeader: 'Unterstrich-Inkonsistenzen:',
    underscoreMismatch: (called, defined) => `  self.${called} aufgerufen, aber ${defined} ist definiert`,
  ...
  cc_extract_classes: {
    ...
    contentHeader: '**Extrahierter Inhalt:**',
    contentTruncated: (maxChars) => `Inhalt nach ${maxChars} Zeichen gekürzt. Nutze output_dir für vollständige Dateien.`,
  ...
  cc_runtime_import_diagnose: {
    description: 'Führt BACH-abgeleitete Python-Runtime-Importdiagnosen in isolierten Subprozessen aus',
    header: (path) => `**Runtime-Importdiagnose: ${path}**`,
    python: 'Python',
    modulesTested: 'Module geprüft',
    importsOk: 'Imports OK',
    failures: 'Fehler',
    circularImports: 'Zirkuläre Imports',
    initFiles: '__init__.py-Dateien',
    timeoutSeconds: 'Timeout-Sekunden',
    noTargets: 'Keine Importziele gefunden. Übergib modules oder zeige path auf ein Python-Projekt.',
    singleImportsHeader: '**Isolierte Imports:**',
    importOk: (target, durationMs) => `- OK ${target} (${durationMs} ms)`,
    importFailed: (target, exitCode, output) => `- FEHLER ${target} (Exit ${exitCode}) ${output}`,
    importTimedOut: (target, durationMs) => `- TIMEOUT ${target} (${durationMs} ms)`,
    initFilesHeader: '**__init__.py-Analyse:**',
    initFileInfo: (file, directImports, hasLazy) =>
      `- ${file}: ${directImports} direkte Imports${hasLazy ? ', Lazy-Imports vorhanden' : ''}`,
    circularHeader: '**Mögliche zirkuläre Imports:**',
    circularPair: (moduleA, moduleB) => `- ${moduleA} <-> ${moduleB}`,
    recommendationsHeader: '**Empfehlungen:**',
    noProblems: '\u2705 Keine Runtime-Importprobleme gefunden.',
    andMore: (count) => `... und ${count} weitere`,
  },

  cc_python_structural_edit: {
    description: 'Prüft und wendet BACH-abgeleitete strukturelle Python-Edits mit Vorschau, Testdatei und Backup-Schutz an',
    header: (path) => `**Struktureller Python-Edit: ${path}**`,
    ...
    classLine: (name, startLine, endLine) => `- Klasse ${name} (Z.${startLine}-${endLine})`,
    functionLine: (name, startLine, endLine) => `- Funktion ${name} (Z.${startLine}-${endLine})`,
    methodLine: (name, startLine, endLine) => `- Methode ${name} (Z.${startLine}-${endLine})`,
    insertedAt: (line) => `Inhalt bei Zeile ${line} eingefügt.`,
    deletedElement: (name, startLine, endLine) => `${name} gelöscht (Z.${startLine}-${endLine}).`,
    replacedLine: (line) => `Zeile ${line} ersetzt.`,
    editFileReady: (path) => `Edit-Datei geschrieben: ${path}`,
    editFilePreview: '**Edit-Datei-Vorschau:**',
    testWritten: (path) => `Testdatei geschrieben: ${path}`,
    backupCreated: (path) => `Backup erstellt: ${path}`,
    appliedTo: (path) => `Angewendet auf: ${path}`,
    diffHeader: '**Vorschau-Diff:**',
    noChanges: 'Keine Änderungen.',
    elementNotFound: (name) => `Element nicht gefunden: ${name}`,
    syntaxFailed: (output) => `Python-Syntaxprüfung fehlgeschlagen: ${output}`,
  },

  // === Tool 16: cc_set_language ===
  cc_set_language: {
    languageSet: (lang) => `Sprache gesetzt auf: ${lang}`,
    languageGet: (lang, supported) => `Aktuelle Sprache: ${lang} (Unterstützt: ${supported.join(', ')})`,
  },
