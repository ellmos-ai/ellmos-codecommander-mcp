...
export const zh: Translations = {
  ...
  cc_analyze_methods: {
    ...
    guardrailsHeader: '**BACH 检查：**',
    missingSignalCallbacksHeader: '缺失的信号回调：',
    missingSignalCallback: (line, method) => `  第 ${line} 行：未找到 self.${method} 回调`,
    attributeIssuesHeader: '属性顺序问题：',
    attributeNeverDefined: (line, attr) => `  第 ${line} 行：self.${attr} 已使用但从未定义`,
    attributeBeforeDefinition: (useLine, attr, defLine) =>
      `  第 ${useLine} 行：self.${attr} 在第 ${defLine} 行定义前被使用`,
    underscoreMismatchesHeader: '下划线不一致：',
    underscoreMismatch: (called, defined) => `  调用了 self.${called}，但定义的是 ${defined}`,
  ...
  cc_extract_classes: {
    ...
    contentHeader: '**提取内容：**',
    contentTruncated: (maxChars) => `内容在 ${maxChars} 个字符后截断。请使用 output_dir 获取完整文件。`,
  ...
  cc_runtime_import_diagnose: {
    description: '在隔离子进程中运行 BACH 派生的 Python 运行时导入诊断',
    header: (path) => `**运行时导入诊断: ${path}**`,
    python: 'Python',
    modulesTested: '已测试模块',
    importsOk: '导入成功',
    failures: '失败',
    circularImports: '循环导入',
    initFiles: '__init__.py 文件',
    timeoutSeconds: '超时秒数',
    noTargets: '未找到导入目标。请传入 modules 或将 path 指向 Python 项目。',
    singleImportsHeader: '**隔离导入:**',
    ...
    initFilesHeader: '**__init__.py 分析:**',
    initFileInfo: (file, directImports, hasLazy) =>
      `- ${file}: ${directImports} 个直接导入${hasLazy ? '，存在惰性导入' : ''}`,
    circularHeader: '**可能的循环导入:**',
    circularPair: (moduleA, moduleB) => `- ${moduleA} <-> ${moduleB}`,
    recommendationsHeader: '**建议:**',
    noProblems: '\u2705 未发现运行时导入问题。',
    andMore: (count) => `... 以及另外 ${count} 项`,
  },

  cc_python_structural_edit: {
    description: '检查并应用 BACH 派生的 Python 结构化编辑，支持预览、测试文件和备份安全模式',
    header: (path) => `**Python 结构化编辑: ${path}**`,
    operationLabel: '操作',
    modeLabel: '模式',
    changedLabel: '已更改',
    syntaxLabel: '语法',
    ...
    backupLabel: '备份',
    structureHeader: '**结构:**',
    classLine: (name, startLine, endLine) => `- 类 ${name} (L.${startLine}-${endLine})`,
    functionLine: (name, startLine, endLine) => `- 函数 ${name} (L.${startLine}-${endLine})`,
    methodLine: (name, startLine, endLine) => `- 方法 ${name} (L.${startLine}-${endLine})`,
    insertedAt: (line) => `已在第 ${line} 行插入内容。`,
    deletedElement: (name, startLine, endLine) => `已删除 ${name} (L.${startLine}-${endLine})。`,
    replacedLine: (line) => `已替换第 ${line} 行。`,
    editFileReady: (path) => `已写入编辑文件: ${path}`,
    editFilePreview: '**编辑文件预览:**',
    testWritten: (path) => `已写入测试文件: ${path}`,
    backupCreated: (path) => `已创建备份: ${path}`,
    appliedTo: (path) => `已应用到: ${path}`,
    diffHeader: '**预览 diff:**',
    noChanges: '没有更改。',
    elementNotFound: (name) => `未找到元素: ${name}`,
    syntaxFailed: (output) => `Python 语法检查失败: ${output}`,
  },
